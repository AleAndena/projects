...
export async function POST(req: Request) {
    try {
        ...
        const questionUserContent = `Given this category/niche: "${niche}", generate 5 generic questions about the best tools or solutions in that category (e.g., What’s the best [niche]?). To refine the niche, use this context but exclude brand-specific temrs: ${contextAboutSiteForAI}. Return only RAW JSON without ANY markdown like this: string[]`;
        const questionSystemContent = 'You’re an SEO expert. Generate 5 generic questions about the best tools or solutions for a given category/niche. Questions must focus on the niche (e.g., What’s the best supermarket shelving? for supermarket equipment) and MUST NOT mention any specific brand, company, or URL. Use the provided context to understand the niche but exclude brand-specific terms. Return RAW JSON without ANY markdown in this format: string[]';
